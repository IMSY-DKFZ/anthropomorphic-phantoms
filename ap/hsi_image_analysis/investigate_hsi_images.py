...
from ap.utils.postprocess_segmentation import filter_small_components
...
hsi = path.read_cube(normalization=1)
...
# vessel_spectrum = np.interp(np.arange(700, 851, 10), wavelengths, vessel_spectrum)
# absorption = - np.log10(vessel_spectrum)
#
# plt.plot(np.arange(700, 851, 10), absorption)
# plt.title(f"sO2: {unmix_so2_proxy(absorption)}")
# plt.show()
...
for image in image_list:
    ...
    path = DataPath(image)
    rgb = path.read_rgb_reconstructed()
    # nrrd.write(os.path.join(image, os.path.basename(image) + ".nrrd"), rgb)
    # features = features_func(rgb)
